...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        for k, sent in enumerate(sents):
            ...
            image_name=os.path.join(eval_kwargs['image_root'], data['infos'][k]['file_path'])
            image_name=image_name.split('/')[-1][:-4]
            if verbose:
                #print('image %s: %s' %(entry['image_id'], entry['caption']))
                print('%s: %s' %(image_name, entry['caption']))
                response[image_name]=entry['caption']
    ...
    model.train()
    return loss_sum/loss_evals, predictions, lang_stats,response
